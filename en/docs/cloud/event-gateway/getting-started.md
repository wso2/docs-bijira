...
```bash
curl -sLO https://github.com/wso2/api-platform/releases/download/event-gateway/v0.7.0/wso2apip-event-gateway-0.7.0.zip && \
unzip wso2apip-event-gateway-0.7.0.zip
```
...
```bash
cat > wso2apip-event-gateway-0.7.0/configs/keys.env << 'ENVFILE'
GATEWAY_CONTROLPLANE_HOST=connect.bijira.dev
GATEWAY_REGISTRATION_TOKEN=<your-gateway-token>
ENVFILE
```

Once you copy the above command from the screen, the `<your-gateway-token>` placeholder will be populated and the `wso2apip-event-gateway-0.7.0/configs/keys.env` file will be created with these environment variables.
...
```bash
cd wso2apip-event-gateway-0.7.0

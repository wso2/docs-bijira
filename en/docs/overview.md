...
### 1. I want a full platform to manage my APIs and AI services
...
* **Option A: Cloud (WSO2 hosts everything)**
    The fastest way to get started. WSO2 runs the control plane (where you design and manage APIs) and the data plane (where traffic flows). Sign up and start creating APIs within minutes. There is no infrastructure to manage.
...
* **Option B: Hybrid (WSO2 hosts the control plane, you host the gateways)**
    You get the convenience of a cloud managed control plane (the UI where you design and govern APIs), but the gateways run in your own data center or cloud account. API traffic never leaves your infrastructure. This satisfies the majority of data residency requirements globally.
...
### 2. I just want a gateway with no platform or UI
...
| Feature | Inside the platform (Cloud, Hybrid, or API Manager) | Standalone |
| :--- | :--- | :--- |
| **How you configure it** | Through the platform web UI | YAML files, CLI, REST API |
| **Control plane** | Included. Manage APIs, policies, subscriptions, and lifecycle visually | None. You manage everything in configuration files |
| **Developer portal** | Included. Developers discover and subscribe to your APIs | None |
| **AI Workspace** | Included. Manage LLM providers, guardrails, and MCP at org level | None. Configure per gateway |
| **Analytics and monetization** | Included (Moesif) | Basic observability (logs, traces) |
...
| :--- | :--- | :--- |
| **API Control Plane** | Design, publish, version, and govern APIs through the web UI. Enforce policies across all connected gateways. | Part of Cloud and API Manager |
...
| **Analytics and Monetization** | Traffic monitoring, runtime and audit logs, usage tracking, and API monetization with usage-based billing. Powered by Moesif. | [Analytics docs →](cloud/api-monetization/overview.md) |

### Gateway components
| Component | What it does | Docs |
| :--- | :--- | :--- |
| **API Gateway** | High-performance, Go-based gateway for API traffic. Handles authentication, rate limiting, transformations, and custom policies. | [Standalone docs →](api-gateway/overview.md) · [Cloud-integrated docs](cloud/api-platform-gateway/getting-started.md) |
| **AI Gateway (LLM Proxy)** | Route, secure, and govern outbound LLM traffic. Includes guardrails (PII masking, prompt injection, content safety), multi-provider load balancing, failover, semantic caching, and cost governance. | [Standalone docs →](ai-gateway/llm-proxy/quick-start-guide.md) · [Cloud-integrated docs](cloud/ai-gateway/overview.md) |
| **AI Gateway (MCP Proxy)** | Govern inbound MCP traffic from AI agents. Generate MCP tools from REST APIs, aggregate tools, and enforce access control and authorization. | [Standalone docs →](ai-gateway/mcp-proxy/quick-start-guide.md) · [Cloud-integrated docs](cloud/mcp-servers/get-started-with-mcp.md) |
| **Third-Party Gateways** | Federate existing gateways (AWS, Azure, Kong, Envoy) into the platform. Discover APIs from these gateways and govern them from the WSO2 control plane without replacing them. | [Federation docs →](cloud/federation/overview.md) |
| **Policy Hub** | API policies and AI guardrails are distributed through the Policy Hub, an open source repository powered by [github.com/wso2/gateway-controllers](https://github.com/wso2/gateway-controllers). The Policy Hub is integrated into Cloud, API Manager (4.7 and above), and the standalone gateways. When a new policy or guardrail is released, it becomes instantly available to all users. | [Policy Hub docs →](policy-hub/overview.md) |

## Choose your path

| I want to... | Start here |
| :--- | :--- |
| Get up and running fast with a managed cloud platform | [Cloud →](cloud/introduction/quick-start-guide.md) |
...
| Migrate from API Manager 4.6 or earlier | [Migration guide →](#) |


## Key concepts
The following concepts apply across the entire platform:
* **Organizations** — The top-level boundary for your platform instance. All APIs, users, and configurations belong to an organization.
* **Projects** — Logical groupings of APIs and services within an organization. Use projects to separate teams, environments, or business domains.
* **Data Planes** — The gateway runtime environments where your APIs are deployed and traffic is processed. A data plane can be WSO2 managed (cloud), self-hosted (hybrid or self-managed), or a federated third-party gateway.

## Already using WSO2?
**API Manager users (4.6 and below):** Your documentation remains at `apim.docs.wso2.com`. Starting with version 4.7 (April 2026), API Manager connects to the new Go-based platform gateway, the same runtime used by Cloud and Hybrid. You can migrate at your own pace: new projects can use the platform gateway while existing APIs continue running on the Java-based universal gateway. APIM 4.7 still ships with the universal gateway if you are not ready to switch. See the API Manager section for 4.7 and above documentation.
